...
class WechatController extends Controller {
  ...
  async sendText () {
    const { ctx, app, logger } = this
    // const { ctx, wechatApi } = this
    console.log('123456sendText')

    const send = await app.wechatApi.sendText(
      'o-J_j07AfzgP7iJGEivbalIMhqzM',
      'test send'
    )

    // const api = new WechatAPI(
    //   app.config.wechatApi.appId,f
    //   app.config.wechatApi.appSecret
    // )

    // var result = await api.sendText(
    //   'o-J_j07AfzgP7iJGEivbalIMhqzM',
    //   'remarked stest send'
    // )
  }
  ...
  async authorizeURL () {
    ...
    const url = app.OAuth.getAuthorizeURL(
      redirect,
      state || '',
      scope || 'snsapi_base'
    )
